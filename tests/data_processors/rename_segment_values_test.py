...
class TestRenameSegmentValues:
    def test__basic_rename(self) -> None:
        stream = get_stream()

        res = stream.rename_segment_values("region", {"north": "North America"})
    ...
    def test__multiple_renames(self) -> None:
        ...
        res = stream.rename_segment_values("region", {"north": "X", "south": "Y"})
    ...
    def test__rename_to_existing_merges_values(self) -> None:
        ...
        res = stream.rename_segment_values("region", {"north": "south"})
    ...
    def test__chains_after_add_clusters(self) -> None:
        """rename_segment_values composes cleanly after add_clusters."""
        df = pd.DataFrame(
            [
        ...
        res = stream.add_clusters(
            ...
            method="kmeans",
            n_clusters=2,
        ).rename_segment_values("cluster", {"cluster_0": "short", "cluster_1": "long"})
    ...
    def test__unknown_segment_col_raises(self) -> None:
        with pytest.raises(PreprocessingConfigError):
            get_stream().rename_segment_values("unknown_col", {"north": "X"})

    def test__unknown_value_raises(self) -> None:
        with pytest.raises(PreprocessingConfigError):
            get_stream().rename_segment_values("region", {"UNKNOWN": "X"})

    def test__empty_mapping_is_noop(self) -> None:
        stream = get_stream()

        res = stream.rename_segment_values("region", {})
    ...
    def test__preserves_categorical_dtype(self) -> None:
        ...
        res = stream.rename_segment_values("region", {"north": "X"})
    ...
    def test__invalid_mapping_type_raises(self) -> None:
        with pytest.raises(PreprocessingConfigError):
            get_stream().rename_segment_values("region", ["north", "X"])

    def test__non_string_values_raises(self) -> None:
        with pytest.raises(PreprocessingConfigError):
            get_stream().rename_segment_values("region", {"north": 123})
