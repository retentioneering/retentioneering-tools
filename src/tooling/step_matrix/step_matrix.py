...
class StepMatrix:
    ...
    export_segments: string (optional, default None),
        Segment name. Option is only for centered stepmatrix. If centered
        step matrix is created, then users selected can be exported as a segment.
    ...
    def __init__(
        ...
        self.groups = groups
        self.export_segments = export_segments
        self.show_plot = show_plot

    def _step_matrix_values(self, data):
        """
        A supplemental function to calculate values for step_matrix
        Parameters
        ----------
        data
        Returns
        ...
        if self.centered is not None:
            ...
            if len(users_to_keep) == 0:
                ...
                if self.export_segments is not None:
            ...
            def pad_to_center(x):
                position = x.loc[(x[self.event_col] == center_event) & (x["occurrence_counter"] == occurrence)][
                    "event_rank"
                ].min()
            ...
            data = data.groupby(self.weight_col).apply(pad_to_center)
        ...
        piv = pad_cols(piv, self.max_steps)
        ...
        if self.targets:
            ...
            piv_targets = pad_cols(piv_targets, self.max_steps)
    ...
    def plot_step_matrix(self, data, targets=None, *, targets_list: list[list[str]] | None = None, title=""):
    ...
    def plot(self):
        ...
        if self.thresh != 0:
            ...
            thresholded = piv.loc[(piv.abs() < self.thresh).all(1)].copy()
            if len(thresholded) > 0:
                piv = piv.loc[(piv.abs() >= self.thresh).any(1)].copy()
        ...
        if self.sorting is None:
            ...
            if {*self.sorting} != {*piv.index}:
                error_text = "The sorting list provided does not match the list of events. "
                error_text += "Run with `sorting` = None to get the actual list"
                raise ValueError(error_text)
        ...
        if self.show_plot:
            ...
            self.plot_step_matrix(piv, piv_targets, targets_list=targets_plot, title=title)
