...
#1. get access to the eventstream columns using further library tools.
    For example in dataprocessors:
    - :py:meth:`positive target dataprocessor<retentioneering.data_processors_lib.positive_target>`
    - :py:meth:`negative target dataprocessor<retentioneering.data_processors_lib.negative_target>`
    - :py:meth:`negative target dataprocessor<retentioneering.data_processors_lib.filter_events>`
    - :py:meth:`negative target dataprocessor<retentioneering.data_processors_lib.group_events>`
...
This can be done with :py:meth:`EventstreamSchema<retentioneering.eventstream.schema.EventstreamSchema>`
class. Here’s how it works. Firstly, let's see once again default ``Eventstream`` displayed columns.

Among the standard triple ``user_id``, ``event``, ``timestamp``
...
See :ref:`explanation of to_dataframe()<to_dataframe explanation>` method.
...
.. code-block:: python
    ...
    Eventstream(df1, schema=new_eventstream_schema).to_dataframe()
...
Now let's look closely which columns are represented in an eventstream and discuss the work of
:py:meth:`to_dataframe()<retentioneering.eventstream.eventstream.Eventstream.to_dataframe>`
...
- ``event_index`` - an integer which is associated with the event order. By default, an eventstream
  ...
  end of a user's path, special sorting is applied. See :doc:`Preprocessing user guide</user_guides/dataprocessors>`
  for the details. :red:`TODO: set a precise link to synthetic events sorting subsection`.
...
The output consists of three data blocks: basic statistics, time distribution and events distribution.
``session_col`` argument points to an eventstream column which contains session ids
(``session_id`` is the default value). If defined, session statistics is also included.
Otherwise all the values related to sessions are not displayed.

By `eventstream start` and `eventstream end` in the "Basic statistics" block we mean timestamps of the
first event and the last events in the eventstream correspondingly. `eventstream length`
is a time distance between event stream start and end. "User path/session time length" shows some
time-based statistics over user paths and sessions. Blocks "User path/session time length" and "Number
of events per user path/session" provides similar information on the length of users paths and sessions
(correspondingly), but the former is calculated in days and the latter in the number of events.
...
:py:meth:`describe_events()<retentioneering.eventstream.eventstream.Eventstream.describe_events>`
...
Since an eventstream has its natural time borders, by lifetime we mean the length of the observed u
ser path as the time distance between the first and the last event represented in the trajectory.
...
- ``lower_cutoff_quantile``, ``upper_cutoff_quantile`` indicates the lower and upper quantiles
...
:py:meth:`timedelta_hist()<retentioneering.eventstream.eventstream.Eventstream.timedelta_hist>` shows
a histogram for the distribution of the timedeltas between a couple of specified events.

The method supports the same formatting arguments (``bins``, ``timedelta_unit``, ``log_scale``,
``lower_cutoff_quantile``, ``upper_cutoff_quantile``) as
:py:meth:`user_lifetime_hist()<retentioneering.eventstream.eventstream.Eventstream.user_lifetime_hist>`.
...
Then we set `event_pair=('product1', 'cart')` and pass it to ``timdelta_hist``:
...
As you may notice from the previous chart, quite many timedeltas have relatively high values. Yes, we
can interpret this in a way like the users are picky, so it takes them long to go from ``product1``
to ``cart`` or probably ``product1`` seems not so popular so the users don't want to purchase it.
Anyway, sometimes we are interested to look only at those events which appeared within a user session.
So if we've already split the paths into sessions we can use ``weight_col='session_id'``:
