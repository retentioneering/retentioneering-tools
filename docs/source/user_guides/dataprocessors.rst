...
We can also run ``LabelLostUsers`` with ``lost_cutoff`` passed, to
arbitrarily label some users as lost. Assume we consider a user
absent if there have been no events for 30 days:

.. code-block:: python

    res = stream.label_lost_users(lost_cutoff=(30, 'D')).to_dataframe()
...
``LabelCroppedPaths`` data processor uses passed ``left_truncated_cutoff`` and
...
with ``cutoff`` parameter. Thus, ``DropPaths`` removes all
...
.. code-block:: python
    ...
    res = stream.drop_paths(events_num=25).to_dataframe()
...
.. code-block:: python
    ...
    res = stream.drop_paths(cutoff=(1, 'M')).to_dataframe()
