...
from typing import MutableMapping, Sequence, Optional, MutableSequence, Any, cast
...
def generateId(size: int = 6, chars: list[str] = string.ascii_uppercase + string.digits):
    return "el" + ''.join(random.choice(chars) for _ in range(size))


def clear_dict(d: dict[Any, Any]) -> dict[Any, Any]:
...
class ReteGraph:
    ...
    def __init__(
            ...
            graph_settings: GraphSettings = None,
    ):
    ...
    def _recalculate(self):
        ...
        updated_clickstream = self.source_clickstream.copy()
        ...
        # save norm type
        recalculated_nodelist = self.preprocessing.create_nodelist(
            updated_clickstream)
        recalculated_edgelist = self.preprocessing.create_edgelist(
    ...
    def _save_html(self, html: str):
        if self.env == "classic":
            ...
            filename = 'graph_{}'.format(datetime.now()).replace(
                ':', '_').replace('.', '_') + '.html'
            path = self.config.experiments_folder + "/" + filename
    ...
    def plot_graph(
        ...
        norm_nodes_threshold = settings[
            "nodes_threshold"] if "nodes_threshold" in settings else self._get_norm_node_threshold(
            nodes_threshold)
        norm_links_threshold = settings[
            "links_threshold"] if "links_threshold" in settings else self._get_norm_link_threshold(
            links_threshold)
        cols = self.config.get_nodelist_cols()
