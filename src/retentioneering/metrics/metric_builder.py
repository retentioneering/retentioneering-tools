...
class MetricConfig:
    ...
    def _parse_dict_config(self, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fields.get("event_names") is None and "event_names" in fields:
            # has_event/event_count wildcard ('events' omitted/None/[]): eagerly
            # resolve to every available event (when known) so 'metric_names'/
            # 'cols' reflect the real columns MetricBuilder will produce instead
            # of staying None. Callers that only need the parsed shape (not the
            # resolved column names) may omit available_events.
            if self.available_events is not None:
                fields = {**fields, "event_names": sorted(self.available_events)}
