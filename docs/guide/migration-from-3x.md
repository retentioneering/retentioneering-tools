...
| `collapse_loops(suffix, time_agg)` | [`collapse_events(loops=, event_groups=, group_col=, ...)`](/docs/data-processors/collapse-events) | Loop-squashing is now one mode of a general merging processor. |
| `group_events` / `group_events_bulk` | [`collapse_events(event_groups=..., name=...)`](/docs/data-processors/collapse-events) or [`rename_events`](/docs/data-processors/rename-events) | Renaming several events to one name is `rename_events`; merging runs of them is `collapse_events`. |
