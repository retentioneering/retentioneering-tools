...
class TestCohorts:
    def test_cohorts__matrix_MM(self, test_stream):
        correct_res = pd.DataFrame(
            ...
            name="CohortGroup"),
            columns=pd.Index([0, 1, 2, 3, 4], dtype='int64',
            name='CohortPeriod')
            )
    ...
    def test_cohorts__matrix_M2M(self, test_stream):
        ...
        [[1.0, 1.0, 0.33],
        [1.0, 0.5, -999.0]],
        index=pd.Index(['2021-12','2022-02'],
        name="CohortGroup"),
        columns=pd.Index([0, 1, 2], dtype='int64',
        name='CohortPeriod')
        )
    ...
    def test_cohorts__matrix_D30D(self, test_stream):
        ...
        [1.0, 0.67, 0.33, -999.0],
        [1.0, 1.0, -999.0, -999.0]],
        index=pd.Index(['2021-12-28', '2022-01-27', '2022-02-26'],
            name="CohortGroup"),
        columns=pd.Index([0, 1, 2, 3], dtype='int64',
            name='CohortPeriod')
        )
    ...
    def test_cohorts__matrix_W4W(self, test_stream):
        correct_res = pd.DataFrame(
            [[1.0, 1.0, -999.0, 0.33],
            [1.0, 0.6667, 0.33, -999.0],
            [1.0, 1.0, -999.0, -999.0]],
            index=pd.Index(['2021-12-27', '2022-01-24', '2022-02-21'],
                           name="CohortGroup"),
            columns=pd.Index([0, 1, 2, 3], dtype='int64',
                             name='CohortPeriod')
        )
    ...
    def test_cohorts__matrix_D4W(self, test_stream):
        correct_res = pd.DataFrame(
        [[1.0, 1.0, -999.0, 0.3333],
        [1.0, 0.6667, 0.3333, -999.0],
        [1.0, 1.0, -999.0, -999.0]],
            index=pd.Index(['2021-12-28', '2022-01-25', '2022-02-22'],
                           name="CohortGroup"),
            columns=pd.Index([0, 1, 2, 3], dtype='int64',
                             name='CohortPeriod')
        )
    ...
    def test_cohorts__matrix_W30D(self, test_stream):
        ...
        [1.0, 0.6667, 0.3333, -999.0],
        [1.0, 1.0, -999.0, -999.0]],
        index = pd.Index(['2021-12-27', '2022-01-26', '2022-02-25'],
                         name="CohortGroup"),
        columns = pd.Index([0, 1, 2, 3], dtype='int64',
                                   name='CohortPeriod')
        )
    ...
    def test_cohorts__matrix_avg(self, test_stream):
        ...
        index = pd.Index(['2021-12-28', '2022-01-18',
                          '2022-02-08', '2022-03-01', 'Average'],
                         name="CohortGroup"),
        columns = pd.Index([0, 1, 2, 3, 4], dtype='int64',
                                   name='CohortPeriod')
        )
        assert run_test(test_stream, correct_res, cohort_start_unit='D',
                 cohort_period=(21, 'D'),
                 average=True,
                 cut_bottom=0,
                 cut_right=0,
    ...
    def test_cohorts__matrix_cut(self, test_stream):
        correct_res = pd.DataFrame(
            [[1.0, 0.83]],
            index=pd.Index(['2021-12-28'],
                           name="CohortGroup"),
            columns=pd.Index([0, 1], dtype='int64',
                             name='CohortPeriod')
        )
        assert run_test(test_stream, correct_res, cohort_start_unit='D',
                 cohort_period=(21, 'D'),
