...
class TestStepMatrix:
    ...
    def test_step_matrix__max_steps_100(
        ...
        assert run_test(stream_simple_shop, max_steps_100_cor, threshold=0, val=0, max_steps=100)
    ...
    def test_step_matrix__differential_targets(
        self, test_stream: EventstreamType, differential_targets_cor: pd.DataFrame
    ):
        g1 = [3, 6]
        g2 = [1, 2, 5]

        assert run_test(
            test_stream, differential_targets_cor, val=0, max_steps=10, targets="event4", groups=(g1, g2), threshold=0
        )
