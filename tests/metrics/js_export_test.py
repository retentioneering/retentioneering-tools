...
editor's METRIC_NAMES) hasn't drifted from metrics/metric_schema.py's
METRIC_SCHEMAS registry — see ADR-0007. If this fails, someone added/renamed/
removed a metric without running `make export-metric-schema`.
"""
...
def test_generated_metric_names_is_up_to_date():
    from retentioneering.metrics.metric_schema import METRIC_SCHEMAS

    expected = _export_metric_schema.render(sorted(METRIC_SCHEMAS))
