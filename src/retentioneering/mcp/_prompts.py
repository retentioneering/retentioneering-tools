...
def _static_instructions(tail: str = "") -> str:
    ...
    lines = [
        ...
        "   get_conversion_rate(start_anchor, end_anchor, within=...) instead of eyeballing",
        ...
        "Call get_conversion_rate(start_anchor='A', end_anchor='B') — one call, no",
        "  preprocessing. within=10 (events) or within='30m' (time) for a window;",
        "  end_anchor=['B','C','path_end'] asks about several outcomes at once.",
        ...
        "Idioms: end_anchor='path_end', within=1 → exit rate after A;",
        "  start_anchor={'pattern': 'path_start->A', 'at': -1} → only paths that STARTED on A;",
        "  start_anchor=end_anchor → how often the event repeats.",
